...
// Structure to represent a node in our priority queue (now stored in a heap)
typedef struct pq_node {
    path data;
} pq_node;

// Structure to represent a priority queue (using a min-heap)
typedef struct priority_queue {
    pq_node *elements; // Array to store heap elements
    int size;          // Current number of elements in the heap
    int capacity;      // Maximum capacity of the heap
} priority_queue;

// Initialize a priority queue (using a min-heap)
priority_queue* pq_init(int capacity) {
    ...
    pq->size = 0;
    pq->elements = (pq_node*)malloc(capacity * sizeof(pq_node));
    if (!pq->elements) {
        perror("Failed to allocate priority queue elements");
        free(pq);
        exit(EXIT_FAILURE);
    }
    return pq;
...
// Helper function to swap two pq_node elements
...
// Insert a path into the priority queue (min-heap based)
void pq_insert(priority_queue *pq, city *city_ptr, city *prev, int dist) {
    ...
    int i = pq->size;
    pq->size++;
    pq->elements[i].data.city = city_ptr;
    pq->elements[i].data.prev = prev;
    pq->elements[i].data.dist = dist;

    // Heapify up
    while (i > 0 && pq->elements[i].data.dist < pq->elements[parent(i)].data.dist) {
        swap(&pq->elements[i], &pq->elements[parent(i)]);
        i = parent(i);
    }
}

// Helper function to maintain the min-heap property (heapify down)
void heapify_down(priority_queue *pq, int i) {
    int left = left_child(i);
    int right = right_child(i);
    int smallest = i;

    if (left < pq->size && pq->elements[left].data.dist < pq->elements[smallest].data.dist) {
        smallest = left;
    }

    if (right < pq->size && pq->elements[right].data.dist < pq->elements[smallest].data.dist) {
        smallest = right;
    }
...
// Remove and return the path with the smallest distance (from min-heap)
...
// Free all memory used by the priority queue (min-heap based)
...
int dijkstra(city *src, city *dst, map *trains) {
    ...
    priority_queue *pq = pq_init(trains->city_count);
