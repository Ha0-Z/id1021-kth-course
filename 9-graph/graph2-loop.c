...
int shortest_path(city *from, city *to, city **path, int k) {
    ...
    // Add current city to path for loop detection
    path[k] = from;
    
    while(nxt != NULL) {
        if (!loop(path, k, nxt->destination)) {
            // We found a city that's not in our current path
            
            // Recursive call to find shortest path from next city
            int d = shortest_path(nxt->destination, to, path, k+1);
            
            // If we found a valid path and it's better than what we had before
